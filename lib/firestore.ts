...
export const formatLandingPageDoc = (id: string, data: any, usePublishedVersion: boolean = false): LandingPage => {
  ...
  const products = rawProducts.map((p: any, idx: number) => ({
    ...
    price: p.price ?? p.individualPrice ?? 109,
    originalPrice: p.originalPrice ?? p.oldPrice ?? 999,
    buttonText: p.buttonText || source.ctaText || 'Get Bundle',
    buttonUrl: p.buttonUrl || source.externalPaymentUrl || 'https://checkout.example.com/pay/109',
  ...
  return {
    ...
    price: products[0]?.price ?? source.price ?? 109,
